...
import { publicSiteCatalog, readSiteCatalog } from "../../catalog/site-catalog.mjs";

const GENERATED_PREFIX = "// AUTO-GENERATED by scripts/generate-workspace-index.mjs. Do not edit.\nexport const WORKSPACE_SNAPSHOT = ";
...
function shouldIgnore(entryName) {
  return (
    IGNORED_DIRS.has(entryName)
    || entryName.endsWith("-worktrees")
    || entryName.startsWith("ragbaz.cc-reconcile-")
  );
}
...
export function comparableSnapshot(snapshot) {
  return JSON.stringify({
    ...
    manifests: snapshot?.manifests ?? [],
    sitePages: snapshot?.sitePages ?? [],
    publicCatalog: snapshot?.publicCatalog ?? null,
...
export function formatGeneratedWorkspaceIndex(snapshot) {
  return (
    `${GENERATED_PREFIX}${JSON.stringify(snapshot, null, 2)}${GENERATED_SUFFIX}`
  );
}

export function readGeneratedWorkspaceIndex(outputPath) {
  try {
    const source = readFileSync(outputPath, "utf8");
    if (!source.startsWith(GENERATED_PREFIX) || !source.endsWith(GENERATED_SUFFIX)) {
      return null;
    }
    return {
      source,
      snapshot: JSON.parse(
        source.slice(GENERATED_PREFIX.length, -GENERATED_SUFFIX.length),
      ),
    };
  } catch {
    return null;
...
export function writeGeneratedWorkspaceIndex(snapshot, outputPath) {
  writeFileSync(outputPath, formatGeneratedWorkspaceIndex(snapshot));
}
